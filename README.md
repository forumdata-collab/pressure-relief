...
| 😊 面部心率 | rPPG：固定 3-ROI GRGB + bandpass + 諧波加成 FFT → BPM（54–180） |
| 👆 手指心率 | 後鏡頭 + 手電筒透射式 PPG（Android），訊號更強 |
...
4. **濾波** — 移動平均 detrend（~1s 窗）→ 一階 HP+LP bandpass **0.9–3.0 Hz**（54–180 BPM）。
5. **峰值** — Hann DFT + **諧波加成評分**（自身能量 + 二次諧波能量；需 ≥10% max 自身門檻防次諧波鎖定）+ 對數拋物線內插亞 bin 精度。
6. **顯示** — 中位數 of 5 平滑；SNR <1dB 及 45–160 BPM 以外丟棄。
...
| 4 | **呼吸諧波搶峰** | 心率長期讀 ~50：呼吸 15-17/min 嘅諧波正正落喺 0.75-0.85Hz | band 下限提到 0.9Hz + 諧波加成評分 |
